...
stdenv.mkDerivation rec {
  ...
  buildPhase = ''
    ...
    dotnet restore --source $TMP/nugetsrc BTCPayServer/BTCPayServer.csproj
    dotnet build -c Release BTCPayServer/BTCPayServer.csproj
  '';

  runScript =  ''
    #!${bash}/bin/bash
    ${dotnetSdk}/bin/dotnet run --no-launch-profile --no-build -c Release \
      -p @@SHARE@@/BTCPayServer/BTCPayServer.csproj -- "$@"
  '';
